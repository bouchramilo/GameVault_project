...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['nom_game']) && isset($_POST['categorie_game']) && isset($_POST['desc_game']) && isset($_POST['prix_game']) && isset($_POST['image_game']) && isset($_POST['time_game'])) {
        $admin->addGame($_POST['nom_game'], $_POST['categorie_game'], $_POST['desc_game'], $_POST['prix_game'], $_POST['image_game'], $_POST['time_game']);
    } elseif (isset($_POST['id_game'])) {
        $admin->deleteGame($_POST['id_game']);
    } elseif (isset($_POST['idgame']) && isset($_POST['title']) && isset($_POST['details']) && isset($_POST['price'])  && isset($_POST['genre'])  && isset($_POST['modifierphoto']) && isset($_POST['modifierTime'])) {
        $admin->updateGame($_POST['idgame'], $_POST['title'], $_POST['details'], $_POST['price'], $_POST['genre'], $_POST['modifierphoto'], $_POST['modifierTime']);
    }
}
...
$favor = new favoris();
// $nbrCritique = $critique->nbrCritique();
    ...
    <div id="formContainerModifier" class="hidden fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full overflow-auto font-[sans-serif]">
        <div class="bg-[#1d1d1d] opacity-90 text-white p-6 flex flex-col gap-6 rounded-lg shadow-lg w-[60%] max-sm:w-full">
            ...
            <form class="space-y-4 mt-8" method="POST">
                ...
                <div>
                    <label class="text-white text-sm mb-2 block">Add Time total</label>
                    <input id="modifierTime" name="modifierTime" type="time" class="px-4 py-3 bg-gray-100 w-full text-gray-800 text-sm border-none focus:outline-blue-600 focus:bg-transparent rounded-lg" />
                </div>
    ...
    <div class="gamemodal hidden fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
        <div class="bg-[#1d1d1d] opacity-90 text-white p-6 flex flex-col gap-6 rounded-lg shadow-lg w-[60%] max-sm:w-full">
            ...
            <form class="space-y-4 mt-8" method="POST">
                ...
                <div>
                    <label for="time_game" class="text-white text-sm mb-2 block">Add background image</label>
    ...
    <!-- <div class=" commentmodal hidden  fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
        <div class="w-full max-w-lg bg-white shadow-lg rounded-lg px-8 py-6 relative">
            ...
            <div class="mt-6 divide-y">
                ...
                $click = "<script>getID();</script>";

                if ($_SERVER['REQUEST_METHOD'] === 'POST' && $click) {
                    $allCritique = $critique->getAllCritiquesForGame($click);
                ...
                ?>
                    <?php foreach ($allCritique as $critique): ?>
                        ...
                        <div class="flex flex-wrap items-center gap-4 py-3 cursor-pointer">
                            ...
                            <div>
                                <p class="text-sm text-gray-800 font-bold"><?php echo $critique['user_full_name'] ?></p>
                                <p class="text-l text-gray-500 mt-0.5"><?php echo $critique['content'] ?></p>
                            </div>
                            <p class="text-xs text-gray-500 mt-0.5 ml-auto"><?php echo $critique['create_at'] ?></p>
                        </div>
